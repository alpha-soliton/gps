...
""" Hyperparameters for MJC musashi peg insertion policy optimization"""
...
SENSOR_DIMS = {
        ...
        ACTION: 7, # shoulder 3 dims and elbow 2 wrist 2 dims
        }

## To be imporved
MUSASHILARM_GAINS = np.array([3, 1, 0.01, 2, 0.1, 1, 1])

BASE_DIR = '/'.join(str.split(gps_filepath, '/')[:-2])
EXP_DIR = BASE_DIR + '/../experiments/musashilarm_badmm/'
...
agent = {
        ...
        'x0': np.concatenate([np.array([-0.5, 0.5, 0, -1.57, 0, 0, 0]), np.zeros(7)]),
        'dt': 0.05,
        'substeps': 5,
        ...
        'obs_include': [JOINT_ANGLES, JOINT_VELOCITIES, END_EFFECTOR_POINTS,
            END_EFFECTOR_POINT_VELOCITIES],
        'camera_pos':np.array([1., 2., 1., -0.1, -0.2, 0.]),
}
...
torque_cost = {
        'type':CostAction,
        'wu': 5e-5/MUSASHILARM_GAINS,
        }

fk_cost = {
    'type': CostFK,
    'target_end_effector': np.array([0.3, 0.3, -0.3, 0.3, 0.3, -0.5]),
